...
export async function genericRateLimit(
  ...
  if (!success) {
    if (diffInSeconds > 60) {
      ...
      return {
        success: false,
        message: `Too many attempts. Please try again in ${resetTimeInMinutes} minute${
          resetTimeInMinutes > 1 ? "s" : ""
        }`,
      };
    }
    ...
    return {
      success: false,
      message: `Too many attempts. Please try again in ${diffInSeconds} minute${
        diffInSeconds > 1 ? "s" : ""
      }`,
    };
  }
